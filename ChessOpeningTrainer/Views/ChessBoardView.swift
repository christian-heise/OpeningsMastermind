...
struct ChessboardView: View {
    ...
    @ObservedObject private var vm_parent: PracticeViewModel
    
    @StateObject private var vm: ChessBoardViewModel
    
    init(vm vm_parent: PracticeViewModel, settings: Settings) {
        self.vm_parent = vm_parent
            ...
            ZStack {
                ForEach(0..<8) {row in
                    ForEach(0..<8) {col in
                        ...
                        if let lastMove = vm_parent.lastMove {
                            if lastMove.to == Square(file: col, rank: 7-row) || lastMove.from == Square(file: col, rank: 7-row) {
                                Rectangle()
                                    .fill(vm_parent.gameState == 1 ? Color.red : Color.yellow)
                                    .frame(width: vm.squareLength(in: geo.size), height: vm.squareLength(in: geo.size))
                                    .position(vm.squarePosition(in: geo.size, col: col, row: row))
                                    .opacity(0.2)
                            }
                        }
                        if row == (vm_parent.userColor == .white ? 7 : 0) {
                            Text(files[col])
                                .rotationEffect(.degrees(vm_parent.userColor == .white ? 0 : 180))
                        ...
                        if col == (vm_parent.userColor == .white ? 0 : 7) {
                            Text(String(8-row))
                                .rotationEffect(.degrees(vm_parent.userColor == .white ? 0 : 180))
                                .position(vm.indicatorPosition(in: geo.size, col: col, row: row))
                                .offset(vm.indicatorOffset(in: geo.size, rowIndicator: false))
                ...
                if vm_parent.gameState == 1 {
                    ForEach(vm_parent.rightMove, id: \.self) { rightMove in
                        ArrowShape()
                ...
                ForEach(vm_parent.pieces, id: \.0) { piece in
                ...
                if let move = vm_parent.last2Moves.0, let annotation = vm.annotations.0 {
                    AnnotationView(annotation: annotation)
                        .frame(width: vm.squareLength(in: geo.size)*0.5)
                        .position(vm.positionAnnotation(move.to, in: geo.size))
                        .zIndex(500)
                }
                if let move = vm_parent.last2Moves.1, let annotation = vm.annotations.1 {
                    AnnotationView(annotation: annotation)
                        .frame(width: vm.squareLength(in: geo.size)*0.5)
                        .position(vm.positionAnnotation(move.to, in: geo.size))
                        .zIndex(500)
                }
                if let move = vm_parent.promotionMove {
                    PawnPromotionView(color: vm_parent.userColor, width: vm.squareLength(in: geo.size)*1.2)
