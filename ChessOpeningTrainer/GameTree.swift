...
class GameTree {
    ...
    static func example() -> GameTree {
        do {
            let node = try decodePGN(pgnString: examplePGN)
            return GameTree(name: "Example", rootNode: node, userColor: .white)
        } catch {
            return GameTree(name: "Example", rootNode: GameNode(moveString: ""), userColor: .white)
        }
    }
}
