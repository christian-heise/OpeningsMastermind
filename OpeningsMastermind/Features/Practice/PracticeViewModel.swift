...
@MainActor class PracticeViewModel: ParentChessBoardModel, ParentChessBoardModelProtocol {
    ...
    override func performMove(_ move: Move) {
        ...
        gameState = .practice
        Task {
            await performComputerMove(in: Int(appData.settings.moveDelay_ms))
        }
    }
...
#if DEBUG
    /// Drives the view model into the `.mistake` review state for the dark-mode
    /// "Learn from your mistakes" App Store screenshot. Walks the Smith-Morra
    /// main line, then plays a plausible but off-book move (8.Nb5?!) so the board
    /// shows the correction arrow and the "not in your studies" banner —
    /// deterministically, without driving a fragile board drag in the UI test.
    /// Mirrors the `.mistake` branch of `performMove(_:)`.
    func setupMistakeScreenshot(in database: DataBase) {
        // Use the exact instance from the database so `PracticeView.onAppear` →
        // `onAppear()` doesn't treat the tree as stale and `reset()` the scene.
        guard let tree = database.gametrees.first(where: { $0.name == "Smith Morra Gambit" }) else { return }

        self.selectedGameTrees = [tree]
        self.userColor = tree.userColor
        self.currentQueueIndex = nil
        self.game = Game(position: startingGamePosition)
        self.moveHistory = []
        self.positionHistory = []
        ...
        // Walk the main line: 1.e4 c5 2.d4 cxd4 3.c3 dxc3 4.Nxc3 Nc6 5.Nf3 d6
        // 6.Bc4 e6 7.O-O Be7 — 14 plies, ending with White (the user) to move.
        var node = tree.rootNode
        for _ in 0..<14 {
            guard let moveNode = node.children.first else { break }
            positionHistory.append(game.position)
            moveHistory.append((moveNode.move, moveNode.moveString))
            positionIndex += 1
            game.make(move: moveNode.move)
            node = moveNode.child
        }
        self.currentNodes = [node]
        self.determineRightMove()

        // Play a deliberately off-book move to trigger the mistake state. Only
        // proceed if it's genuinely not one of the book replies here.
        let wrongMove = SanSerialization.default.move(for: "Nb5", in: game)
        guard game.legalMoves.contains(wrongMove),
              !node.children.contains(where: { $0.move == wrongMove }) else { return }

        let san = SanSerialization.default.correctSan(for: wrongMove, in: game)
        self.gameState = .mistake
        self.positionHistory.append(game.position)
        self.game.make(move: wrongMove)
        self.moveHistory.append((wrongMove, san))
        self.positionIndex += 1
        self.addMistake(true)
    }
#endif
