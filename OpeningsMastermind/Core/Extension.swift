...
extension ProcessInfo {
    /// True when running inside a SwiftUI preview's sandboxed agent process.
    /// Used to skip work that crashes or hangs there (e.g. spawning Stockfish).
    static var isRunningInPreviews: Bool {
        processInfo.environment["XCODE_RUNNING_FOR_PREVIEWS"] == "1"
    }
}

extension PieceColor: @retroactive CaseIterable {
    public static let allCases: [PieceColor] = [.white, .black]
}
