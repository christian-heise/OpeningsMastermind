...
struct ChessboardView<ParentVM>: View where ParentVM: ParentChessBoardModelProtocol {
    ...
    init(vm vm_parent: ParentVM) where ParentVM: ParentChessBoardModelProtocol {
            ...
            GeometryReader { geo in
                ZStack {
                    ForEach(0..<8) {row in
                        ForEach(0..<8) {col in
                            Rectangle()
                                .fill((row + col) % 2 == 0 ? appData.settings.boardColorRGB.white.getColor() : appData.settings.boardColorRGB.black.getColor())
                            ...
                            if row == (vm.userColor == .white ? 7 : 0) {
                                Text(files[col])
                                    ...
                                    .foregroundColor((row + col) % 2 == 0 ? appData.settings.boardColorRGB.black.getColor() : appData.settings.boardColorRGB.white.getColor())
                            }
...
struct ChessboardViewOld_Previews: PreviewProvider {
    static var previews: some View {
        ChessboardView(vm: PracticeViewModel(database: DataBase(), appData: AppData()))
            .environment(AppData())
    }
}
