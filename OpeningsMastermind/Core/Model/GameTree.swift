...
class GameTree: Codable, Hashable, Identifiable {
    ...
    init(name: String, pgnString: String, userColor: PieceColor) {
        ...
        let allNodesArray = decoder.decode(pgnString: pgnString).nodes
        self.rootNode = allNodesArray.first ?? GameNode(fen: startingFEN)
        self.allGameNodes = GameTree.sortedByNodesBelow(allNodesArray, from: self.rootNode)
    ...
    static func getAllNodes(node: GameNode) -> [GameNode] {
        let allNodes = node.reachableNodesPostOrder().filter { $0 !== node }
        return sortedByNodesBelow(allNodes, from: node)
    }

    /// Sorts `nodes` (all reachable from `root`) by `nodesBelow` descending, computing
    /// `nodesBelow` for the whole tree in a single pass instead of per comparison.
    static func sortedByNodesBelow(_ nodes: [GameNode], from root: GameNode) -> [GameNode] {
        let nodesBelowMap = GameNode.nodesBelowMap(from: root)
        return nodes.sorted(by: {
            (nodesBelowMap[ObjectIdentifier($0)] ?? 0) > (nodesBelowMap[ObjectIdentifier($1)] ?? 0)
        })
    }
